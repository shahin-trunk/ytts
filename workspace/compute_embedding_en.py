...
MAX_SAVE_LIMIT = 50000

os.makedirs(SPK_EMB_CACHE_PATH, exist_ok=True)

SPEAKER_ENCODER_CHECKPOINT_PATH = "expmt/se/multi/v9/run-August-30-2023_09+59PM-452d4855/checkpoint_69000.pth"
SPEAKER_ENCODER_CONFIG_PATH = "expmt/se/multi/v9/run-August-30-2023_09+59PM-452d4855/config.json"

manifests_map = {"en_cmb": ["data/audio/multi_lang/sel/en/manifest_en_ns_v1.json"]}
...
for d_name, manifests in manifests_map.items():
    ...
    with open(d_vector_cache_path, encoding="utf-8") as dvcp:
        d_vector_files = json.load(dvcp)

        spk_mapping = {}
        for dv_file in d_vector_files:
            spk_mapping.update(dict(load_file(dv_file)))

        mapping_file_path = os.path.join(d_cache_dir, f"spk_emb_{d_name}_all.pth")
        save_file(spk_mapping, mapping_file_path)
